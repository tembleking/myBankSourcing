...
func (e *EventStore) LoadEventStream(ctx context.Context, aggregateId string) (*EventStream, error) {
	return e.LoadEventStreamSubset(ctx, aggregateId, 0, 0)
}
...
func (e *EventStore) LoadEventStreamSubset(ctx context.Context, aggregateId string, startVersion uint64, maxCount uint64) (*EventStream, error) {
	records, err := e.appendOnlyStore.ReadRecords(ctx, aggregateId, startVersion, maxCount)
	if err != nil {
...
func (e *EventStore) AppendToStream(ctx context.Context, aggregateId string, expectedVersion uint64, events []domain.Event) error {
	if len(events) == 0 {
		return nil
	}
	...
	err = e.appendOnlyStore.Append(ctx, aggregateId, data, expectedVersion)
	if err != nil {
